...
if (isset($_SESSION['admin_id']) && 
    ...
    if ($_SESSION['role'] == 'Admin') {
       ...
       include "data/batch.php"; // Change this line to include the appropriate file for batches
       
       $course_id = $_GET['course_id'];
       $course = getSubjectById($course_id, $conn);
       $batches = getAllBatches($conn); // Change this line to get batches
        ...
        <div class="mb-3">
          ...
          <select name="batch" class="form-control" >
                  <?php foreach ($batches as $batch) { 
                     $selected = 0;
                     if ($batch['batch_id'] == $course['batch'] ) {
                       $selected = 1;
                     }
                  ?>

                    <option  value="<?=$batch['batch_id']?>" <?php if ($selected) echo "selected"; ?> >
